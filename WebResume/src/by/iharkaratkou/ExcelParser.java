...
/**
 * This class contains methods which parse data on Excel sheets and call methods
 * to write info in database.
 * 
...
public class ExcelParser {
	...
	public LinkedHashSet<String> getSheetsNames() {
		...
		sheetsNames.addAll(Arrays.asList("General Data",
				"Summary of qualifications", "Experience", "Certification",
				"Education", "Visited countries"));
		return (LinkedHashSet<String>) sheetsNames;
	}

	public Integer parseExcelToDatabase(String webID, String filenameTimestamp,
			String webPass, boolean newWebResume, String saveFile) {
		Integer id_last = 0;
		Integer id_last_temp = 0;
		// Check insert of update
		if (!webID.equals(""))
			id_last_temp = Integer.valueOf(webID);
	...
	public Integer parseExcelSheetToDatabase(XSSFSheet sheet, String sheetName,
		...
		switch (sheetName) {
		case "General Data":
			id_last = parseExcelSheetGenDataToDatabase(sheet, id_last_temp,
					webPass);
			break;
	...
	public Integer parseExcelSheetGenDataToDatabase(XSSFSheet sheet,
		...
		logger.debug("gdID: " + gdID + " Name: " + Name + " Surname: "
				+ Surname);
		return gdID;
	}
	...
	public void parseExcelSheetCertToDatabase(XSSFSheet sheet,
		...
		while (rowIterator.hasNext()) {
			...
			while (cellIterator.hasNext()) {
				Cell cell = cellIterator.next();
				String cellValue = getExcelValue(cell);
				if (!cellValue.equals("Certification name")
						&& !cellValue.equals("Date") && !cellValue.isEmpty()) {
					certification_temp.add(cellValue);
				}
			}
			if (certification_temp.size() > 0) {
				certifications.add((ArrayList<String>) jhu
						.deepClone(certification_temp));
			}
	...
	public void parseExcelSheetEducToDatabase(XSSFSheet sheet,
		...
		while (rowIterator.hasNext()) {
			...
			while (cellIterator.hasNext()) {
				Cell cell = cellIterator.next();
				String cellValue = getExcelValue(cell);
				if (!cellValue.equals("Diploma")
						&& !cellValue.equals("Educational center")
						&& !cellValue.equals("Period") && !cellValue.isEmpty()) {
					education_temp.add(cellValue);
				}
			}
			if (education_temp.size() > 0) {
				educations.add((ArrayList<String>) jhu
						.deepClone(education_temp));
			}
	...
	public void parseExcelSheetVisCountToDatabase(XSSFSheet sheet,
		...
		while (rowIterator.hasNext()) {
			...
			while (cellIterator.hasNext()) {
				...
				if (!cellValue.equals("Visited countries IDs")
						&& !cellValue.equals("Visited countries")
						&& !cellValue.isEmpty() && !cellValue.equals("0")) {
					vis_countries_temp.add(cellValue);
				}
			}
			if (vis_countries_temp.size() > 0) {
				vis_countries.add((ArrayList<String>) jhu
						.deepClone(vis_countries_temp));
			}
